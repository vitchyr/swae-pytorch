...
from swae.models.custom import PyBulletAutoencoder
from swae.trainer import SWAEBatchTrainer


def main(paths):
    # train args
    parser = argparse.ArgumentParser(description='Sliced Wasserstein Autoencoder PyTorch PyBullet')
    ...
    custom_dataset = load_np_images(paths)
    ...
    model = PyBulletAutoencoder(embedding_dim=args.embedding_size).to(device)
    print(model)
    # create optimizer
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, x in enumerate(train_loader, start=0):
        ...
        with torch.no_grad():
            for test_batch_idx, x_test in enumerate(test_loader, start=0):
...
def load_np_images(
    ...
    for path in paths:
        ...
        all_imgs.append(data[image_key])
    imgs = torch.from_numpy(np.vstack(all_imgs)).to(torch.float)
    return imgs


if __name__ == '__main__':
    paths = [
        '/home/vitchyr/mnt/log/manual-upload/pybullet-dataset/objs_random_12800_1obj.npy',
        '/home/vitchyr/mnt/log/manual-upload/pybullet-dataset/objs_in_bowl_12800_1obj.npy',
    ]
    main(paths)
